package com.example.circlelayoutmanager;
 ...
 */
public class CircleZoomLayoutManager extends RecyclerView.LayoutManager {
    ...
    public CircleZoomLayoutManager(Context context, int itemSpace) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            if (itemsX.get(i) - offsetScroll + startLeft <= getHorizontalSpace()
                ...
                if (!itemAttached.get(i)) {
                    ...
                    float rot = calculateRot(startLeft+x);
                    float translationX = rot * -200;
                    float translationY = Math.abs(rot) * 200;
                    float z = Math.abs(rot) * -1;
                    float rotation=rot*30;
//                    Log.d("l:",i+"i,s:"+scale+",X:"+translationX+"Y"+translationY+"r:"+rot);
                    scrap.setTranslationX(translationX);
                    scrap.setTranslationY(translationY);
                    scrap.setRotation(rotation);
                    scrap.setZ(z);
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for(int i = 0;i<getChildCount();i++){
            ...
            float rot = calculateRot(v.getLeft());
            float translationX = rot * -200;
            float translationY = Math.abs(rot) * 200;
            float z = Math.abs(rot) * -1;
            float rotation=rot*30;
//            Log.d("l:",i+"i,s:"+scale+",X:"+translationX+"Y"+translationY+"r:"+rot);
    ...
    private float calculateRot(int x){
        int deltaX = Math.abs(x-(getHorizontalSpace() - mDecoratedChildWidth) / 2);
        float dir = (x>(getHorizontalSpace() - mDecoratedChildWidth) / 2)?1:-1;

        return dir*((maxScale-1f)/mDecoratedChildWidth * deltaX);
    }
    ...
    public void smoothScrollToPosition(RecyclerView recyclerView, RecyclerView.State state, int position) {
        LinearSmoothScroller smoothScroller = new LinearSmoothScroller(context) {
            ...
            public PointF computeScrollVectorForPosition(int targetPosition) {
                return CircleZoomLayoutManager.this.computeScrollVectorForPosition(targetPosition);
            }
        };
